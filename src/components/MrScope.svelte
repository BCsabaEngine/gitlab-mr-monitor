...
{#await mrPromises then}
	{#if mrs && mrs.length > 0}
		<div class="container mx-auto pb-4">
			...
			<div
				class="flex items-center ml-2 text-lg dark:text-gray-200 cursor-pointer"
				on:click={() => (blockOpen = !blockOpen)}
			>
				{#if blockOpen}
					<CaretDownSolid class="mr-2" />
				{:else}
					<CaretRightSolid class="mr-2" />
				{/if}
				{scope.name}
				<span class=" text-sm mt-0.5 ml-2">
					{mrs.length}
					{#if !blockOpen && arrivalMrsIds.length > 0}
						(!)
					{/if}
				</span>
			</div>
